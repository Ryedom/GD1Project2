...
// enemyScript contains the script for an individual enemy
let enemyScript = function(){
...
enemyScript.prototype.preload = function(){
  game.load.spritesheet('dude', 'assets/dude.png', 32, 48);
  game.load.spritesheet('bullet', 'assets/rgblaser.png', 4, 4);
};

enemyScript.prototype.create = function(x, y, type, playerRef){

  this.enemy = game.add.sprite(x, y, 'dude');
  // additional data
  ...
  if ((this.enemyType === "bully") || (this.enemyType === "football_player_left") || (this.enemyType === "football_player_right")){
    ...
    this.enemyWeapon.trackSprite(this.enemy, this.enemy.width/2, 0, false);
    //END WEAPON----------------------------------------------------------------------
  }
  return this.enemy;
};

enemyScript.prototype.update = function(){
    console.log("enemy updating");
    if (this.enemy.alive){
      if (this.enemyType === "skater"){
        // skaters move towards player
        this.moveEnemyTowardPlayer(this.playerRef.x, this.playerRef.y);
...
enemyScript.prototype.getAngleToPlayer = function(pl_x, pl_y) {
  return Math.atan2(pl_y - this.enemy.y, pl_x - this.enemy.x);

}

// getDistanceFromPlayer - returns a distance (non-negative) from an enemy to a player
enemyScript.prototype.getDistanceFromPlayer = function(pl_x, pl_y) {
...
enemyScript.prototype.moveEnemyTowardPlayer = function(pl_x, pl_y){
  // console.log("moveEnemyTowardPlayer called");
